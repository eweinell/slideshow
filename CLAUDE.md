...
| **Manuelle Reihenfolge** | **Stufe 1 umgesetzt** ([`docs/briefing-manuelle-reihenfolge.md`](docs/briefing-manuelle-reihenfolge.md)). `order.yaml` → `build --order` → `edit.yaml`; Auflösung in `order.py`, alle drei Fehlerfälle mit Datei und Zeile, `rest: error\|append\|drop`. Offen ist Stufe 2: der Generator `slideshow order` (samt `--update`) und der `group:`-Anker in `chapters.yaml` — bis dahin wird die Datei von Hand geschrieben. |
