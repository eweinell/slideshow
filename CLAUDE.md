...
| **Blendenmodus wird nicht geprüft** | **Konzept, offen** ([`docs/briefing-blendenmodus.md`](docs/briefing-blendenmodus.md)). `_XFADE_MODES.get(mode, "fade")` in `kenburns.py` macht aus einem vertippten Modus stillschweigend eine normale Blende — die einzige Stelle im Schema ohne Validierung. `known_modes()` gibt es bereits; die Prüfung gehört als `field_validator` an beide `mode`-Felder in `models.py` (Import im Funktionskörper wegen des Zyklus `kenburns` → `models`). |
| **Titelfolien-Hintergrund nach Tragfähigkeit** | **Konzept, offen** ([`docs/briefing-titelfolien-hintergrund.md`](docs/briefing-titelfolien-hintergrund.md), aus Abschnitt 14.1 des Ken-Burns-Briefings ausgekoppelt — der Teil ohne API). `bg: auto` misst die nötige Abdunklung des ersten Abschnittsbildes (`_fit_darkening` liegt vor); erst wenn sie unter eine Schwelle fällt, gewinnt das tragfähigste der ersten fünf Bilder. Der Vorrang des ersten Bildes ist Absicht: eine andere Wahl schaltet die Fokusblende ab (`_ist_fokusblende` vergleicht `title.bg` mit dem Folgebild) und muss deshalb gemeldet werden. |
