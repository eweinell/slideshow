...
| **Titel- und Zwischenfolien** | **Einbettung vollständig** ([`docs/briefing-titelfolien.md`](docs/briefing-titelfolien.md), Stufe 2). `chapters.yaml` → `type: title`, Phrasenlage, Stille-Regel, Fokusblende, Rundlauf, Deckungsrechnung, `ensure_title_assets` in `render`/`export-mlt`, MLT. **Offen: allein der Generator** — `titles.render_title` wirft noch mit klarer Meldung. Die Naht dorthin steht (Schriftfindung, Layoutparameter, Assetpfad, Frische-Schlüssel); wer sie füllt, fasst nur `titles.py` und eine Zeile in `doctor.py` an. Danach Stufe 3, die Kapitelerkennung `slideshow chapters`. |
