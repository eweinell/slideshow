...
| **Titel- und Zwischenfolien** | **Einbettung umgesetzt** ([`docs/briefing-titelfolien.md`](docs/briefing-titelfolien.md), Stufe 2). `chapters.yaml` → `type: title` in der Edit-List, Phrasenlage, Stille-Regel, Fokusblende, Rundlauf, Deckungsrechnung. **Offen: der Generator selbst** (`titles.render_title` wirft noch) und die Kapitelerkennung `slideshow chapters`. `build` schreibt also bereits vollständige Titelfolien, `render` kann sie mangels Asset noch nicht bauen. |
