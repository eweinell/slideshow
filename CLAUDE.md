...
| **Manuelle Reihenfolge** | **Briefing vorhanden, nicht umgesetzt** ([`docs/briefing-manuelle-reihenfolge.md`](docs/briefing-manuelle-reihenfolge.md)). `build` leitet die Abfolge aus `chronological(manifest)` ab; eine thematische Sortierung geht nur von Hand in `edit.yaml` und stirbt beim nächsten `build`. Vorgesehen ist `order.yaml` nach dem Muster von `chapters.yaml`. Der Parameter `build_edit_list(order=…)` existiert bereits, hat aber keinen Aufrufer und überspringt unbekannte IDs stillschweigend. |
